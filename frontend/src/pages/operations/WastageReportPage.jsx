...
import {
  Typography, Grid, Card, CardContent, Alert, Chip,
} from "@mui/material";
  ...
  return (
    <Layout>
      ...
      <PaginatedTable
        loading={loading}
        rows={rows}
        getRowKey={(r) => r.outlet_id}
        emptyText="No wastage data in this window."
        columns={[
          {
            header: "Outlet",
            render: (r) => (
          ...
          { header: "Damage",           align: "right", render: (r) => lkr(r.damage_value) },
          { header: "Office",           align: "right", render: (r) => lkr(r.office_value) },
          { header: "Verification",     align: "right", render: (r) => lkr(r.verification_value) },
          { header: "Total Wastage",    align: "right", render: (r) => lkr(r.total_wastage),
            cellStyle: () => ({ fontWeight: 600, color: "#b91c1c" }) },
          { header: "Purchases (GRN)",  align: "right", render: (r) => lkr(r.grn_value),
            cellStyle: () => ({ color: "rgba(0,0,0,0.6)" }) },
          { header: "Wastage %",        align: "right", render: (r) =>
