"""Sales Returns (With Reasons) upload pipeline."""
...
def _batch_summary(b: SalesReturnUploadBatch) -> dict:
    return {
        "id": b.id,
...
def _scope_batches(request):
    user = request.user
    qs = SalesReturnUploadBatch.objects.select_related("outlet", "uploaded_by", "approved_by")
    ...
    if not include_deleted:
        qs = qs.exclude(status=SalesReturnUploadBatch.Status.DELETED)
    return qs


def _find_overlapping(outlet, date_from, date_to, exclude_id=None):
    qs = SalesReturnUploadBatch.objects.filter(
        outlet=outlet,
        status=SalesReturnUploadBatch.Status.SUCCESS,
...
def _commit_batch(outlet, user, parsed, filename, approval_status, approved_by=None, approved_at=None):
    with transaction.atomic():
        batch = SalesReturnUploadBatch.objects.create(
            ...
            total_amount=Decimal(str(round(sum(r.gross_value for r in parsed.rows), 2))),
        ...
        lines = [
            SalesReturnLine(
                ...
                gross_value=Decimal(str(r.gross_value)),
                remarks=r.remarks,
                user_name=r.user_name,
            )
            for r in parsed.rows
        ]
        SalesReturnLine.objects.bulk_create(lines, batch_size=500)
        AuditLog.objects.create(
            user=user, action="sales_returns_upload",
...
def _line_dict(l):
    return {
        ...
        "gross_value": float(l.gross_value),
        "remarks": l.remarks,
...
def sales_returns_validate(request):
    ...
    result = validate_sales_returns_file(file, file.name)
...
def sales_returns_confirm(request):
    ...
    validation = validate_sales_returns_file(file, file.name)
...
@api_view(["GET"])
@permission_classes([IsStoreUser])
def sales_returns_batches(request):
    user = request.user
...
@api_view(["GET"])
@permission_classes([IsStoreUser])
def sales_returns_stats(request):
    user = request.user
    qs = _scope_batches(request)
    agg = qs.aggregate(
        total=Count("id"),
        pending=Count("id", filter=Q(approval_status=SalesReturnUploadBatch.ApprovalStatus.PENDING)),
    )
...
@api_view(["GET"])
@permission_classes([IsStoreUser])
def sales_returns_batch_detail(request, batch_id: int):
    batch = get_object_or_404(
        SalesReturnUploadBatch.objects.select_related("outlet", "uploaded_by", "approved_by"),
        pk=batch_id,
    )
    user = request.user
    ...
    line_qs = SalesReturnLine.objects.filter(batch=batch).order_by("txn_date", "invoice_no", "item_code")
    total = line_qs.count()
    offset = (page - 1) * page_size
...
def sales_returns_approve(request, batch_id: int):
    batch = get_object_or_404(SalesReturnUploadBatch, pk=batch_id)
    if batch.approval_status != SalesReturnUploadBatch.ApprovalStatus.PENDING:
    ...
    with batch.stored_file.open("rb") as f:
        parsed = parse_sales_returns_xls(f, batch.filename or "sales_returns.xls")
...
@api_view(["POST"])
@permission_classes([IsAdmin])
def sales_returns_reject(request, batch_id: int):
    batch = get_object_or_404(SalesReturnUploadBatch, pk=batch_id)
    if batch.approval_status != SalesReturnUploadBatch.ApprovalStatus.PENDING:
        return Response({"detail": "Batch is not pending approval."}, status=status.HTTP_400_BAD_REQUEST)

    batch.approval_status = SalesReturnUploadBatch.ApprovalStatus.REJECTED
    batch.approved_by = request.user
    batch.approved_at = timezone.now()
    batch.status = SalesReturnUploadBatch.Status.DELETED
    if batch.stored_file:
        batch.stored_file.delete(save=False)
...
@api_view(["GET"])
@permission_classes([IsStoreUser])
def sales_returns_deletion_preview(request, batch_id: int):
    batch = get_object_or_404(SalesReturnUploadBatch, pk=batch_id)
    can, admin = _can_delete(request.user, batch)
    row_count = SalesReturnLine.objects.filter(batch=batch).count()
    total = SalesReturnLine.objects.filter(batch=batch).aggregate(s=Sum("gross_value"))["s"] or 0
    return Response({
        "batch": _batch_summary(batch),
        "row_count": row_count,
...
@api_view(["DELETE"])
@permission_classes([IsStoreUser])
def sales_returns_delete(request, batch_id: int):
    batch = get_object_or_404(SalesReturnUploadBatch, pk=batch_id)
    ...
    row_count = SalesReturnLine.objects.filter(batch=batch).count()
    with transaction.atomic():
        SalesReturnLine.objects.filter(batch=batch).delete()
        batch.status = SalesReturnUploadBatch.Status.DELETED
        if batch.stored_file:
            batch.stored_file.delete(save=False)
    ...
    AuditLog.objects.create(
        user=request.user, action="sales_returns_upload_deleted",
        entity_type="sales_returns_batch", entity_id=str(batch.id),
        details={
...
def sales_returns_overview(request):
    today = date.today()
    try:
    ...
    batches = SalesReturnUploadBatch.objects.filter(
        status=SalesReturnUploadBatch.Status.SUCCESS,
